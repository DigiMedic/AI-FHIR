...
class DigiMedicAPIClient:
    ...
    def __init__(self, api_base_url: str = None, auth_token: str = None, simulate: bool = True):
        ...
        if not self.simulate:
            ...
            if api_base_url and auth_token:
                ...
                print("VAROVÁNÍ [API Client]: Reálný režim je aktivní, ale chybí konfigurace API (URL nebo token) jak v argumentech, tak v proměnných prostředí. Používají se simulované hodnoty jako fallback.")
        ...
        print(f"INFO [API Client]: Klient inicializován. Režim: {'Simulovaný' if self.simulate else 'Reálný'}.")
        print(f"INFO [API Client]: API Base URL: {self.base_url} ({source_info}).")
        print(f"INFO [API Client]: Auth Token: {token_display}.")
    ...
    def send_fhir_resource(self, fhir_resource: dict) -> dict:
        ...
        if self.simulate:
            ...
            if not resource_type:
                print("CHYBA [API Client]: Chybí 'resourceType' v odesílaném FHIR zdroji.")
                return {
            ...
            print(f"INFO [API Client]: REÁLNÉ ODESLÁNÍ ({method}) FHIR zdroje '{resource_type}{'/' + resource_id if resource_id else ''}' na {url}")
            # Logování části dat pro debug (např. bez citlivých polí, pokud by byla)
            # print(f"DEBUG [API Client]: Data k odeslání (část):\n{json.dumps({k:v for k,v in fhir_resource.items() if k != 'photo'}, indent=2, ensure_ascii=False)}")
            ...
            try:
                ...
                try:
                    ...
                    print(f"INFO [API Client]: Úspěšná odpověď ({response.status_code}) z {method} {url}:\n{json.dumps(response_json, indent=2, ensure_ascii=False)}")
                    return response_json
                except json.JSONDecodeError:
                    print(f"INFO [API Client]: Úspěšná odpověď ({response.status_code}), ale tělo odpovědi není platný JSON. Text odpovědi: {response.text}")
                ...
                print(f"CHYBA [API Client]: HTTP volání ({method}) na {url} vypršelo.")
                ...
                print(f"CHYBA [API Client]: HTTP chyba ({method}) při volání na {url}: {e.response.status_code} {e.response.reason}")
                try:
                    # Pokusit se vrátit OperationOutcome z těla odpovědi serveru
                    error_response_json = e.response.json()
                    print(f"DEBUG [API Client]: Tělo chybové odpovědi (HTTPError):\n{json.dumps(error_response_json, indent=2, ensure_ascii=False)}")
                    ...
                    print(f"DEBUG [API Client]: Tělo chybové odpovědi (HTTPError) nebylo JSON: {e.response.text}")
                ...
                print(f"CHYBA [API Client]: Obecná chyba HTTP volání ({method}) na {url}: {e}")
    ...
    def send_fhir_bundle(self, fhir_bundle: dict) -> dict:
        ...
        if self.simulate:
            ...
            print(f"DEBUG [API Client]: Bundle obsahuje {num_entries} záznamů.")
            ...
            try:
                ...
                try:
                    response_json = response.json()
                    print(f"INFO [API Client]: Úspěšná odpověď ({response.status_code}) z POST {url} pro Bundle:\n{json.dumps(response_json, indent=2, ensure_ascii=False)}")
                    return response_json
                except json.JSONDecodeError:
                    print(f"INFO [API Client]: Úspěšná odpověď ({response.status_code}) pro Bundle, ale tělo odpovědi není platný JSON. Text odpovědi: {response.text}")
                    if response.status_code == 204: # No Content
                ...
                print(f"CHYBA [API Client]: HTTP volání na {url} pro Bundle (ID: {bundle_id}) vypršelo.")
                ...
                print(f"CHYBA [API Client]: HTTP chyba při volání na {url} pro Bundle (ID: {bundle_id}): {e.response.status_code} {e.response.reason}")
                try:
                    error_response_json = e.response.json()
                    print(f"DEBUG [API Client]: Tělo chybové odpovědi (HTTPError) pro Bundle:\n{json.dumps(error_response_json, indent=2, ensure_ascii=False)}")
                    if error_response_json.get("resourceType") == "OperationOutcome":
                        return error_response_json
                    else:
                    ...
                    print(f"DEBUG [API Client]: Tělo chybové odpovědi (HTTPError) pro Bundle nebylo JSON: {e.response.text}")
                ...
                print(f"CHYBA [API Client]: Obecná chyba HTTP volání na {url} pro Bundle (ID: {bundle_id}): {e}")
...
if __name__ == '__main__':
    ...
    print("INFO: Pro reálné testy musí být nastaveny proměnné prostředí DIGIMEDIC_API_BASE_URL a DIGIMEDIC_API_TOKEN.")
    ...
    if real_api_url_from_env and real_api_token_from_env:
        print(f"Nalezeny proměnné prostředí: DIGIMEDIC_API_BASE_URL, DIGIMEDIC_API_TOKEN. Inicializuji klienta v reálném režimu.")
        ...
        # print(f"Odpověď (args): {json.dumps(response_real_patient_args, indent=2, ensure_ascii=False)}")

    else:
        print("VAROVÁNÍ: Proměnné prostředí DIGIMEDIC_API_BASE_URL a/nebo DIGIMEDIC_API_TOKEN nejsou nastaveny.")
        print("Reálné testy budou přeskočeny. Klient bude inicializován s fallback hodnotami (pravděpodobně simulovanými).")
